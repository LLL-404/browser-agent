...
from .city_codes import get_city_code, get_rent_reference
...
from .pre_filter import should_skip_job
from .keyword_strategy import get_keywords_for_city, get_initial_keyword
from .anti_detect import (
    random_delay, human_scroll, ANTI_REDIRECT_SCRIPT,
    STEALTH_SCRIPT, build_browser_kwargs,
)
from .page_analyzer import analyze_page, quick_check
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if concurrent and total > 1:
                ...
                tasks = [_search(city, page) for city, page in zip(cities, pages)]
...
async def health_check() -> dict:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            checks["network"] = {"ok": resp.status == 200, "status": resp.status}
            if resp.status != 200:
                issues.append(f"BOSS直聘返回状态码 {resp.status}")
    except _PLAYWRIGHT_ERRORS as e:
