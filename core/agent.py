...
from .error_handler import format_error_for_mcp
...
class BrowserAgent:
    ...
    async def type_text(self, selector: str,
        ...
        if clear:
            await self._ctrl.click_selector(sel)
            await asyncio.sleep(0.1)
            await self._ctrl.execute_js(
    ...
    async def select(self, selector: str,
        ...
        result = await self._ctrl.execute_js(js)
        if isinstance(result, dict):
            return {"ok": "error" not in result,
                    "selector": sel, "value": value,
                    "step": self._step_count}
        return {"ok": True, "selector": sel, "value": value,
                "step": self._step_count}
    ...
    async def snapshot(self,
                       max_length: int = 3000) -> dict[str, Any]:
        """获取当前页面结构快照（DOM + 可交互元素树 + 文本摘要）。"""
    ...
    async def html(self, max_len: int = 8000) -> dict[str, Any]:
        return {"html": await self._ctrl.get_page_html(max_len)}

    async def text(self, max_len: int = 3000) -> dict[str, Any]:
        return {"text": await self._ctrl.get_page_text(max_len)}
    ...
    async def extract(self, selector: str,
        ...
        field_js_parts = []
        for f in fields:
            if f in ("innerText", "textContent"):
                field_js_parts.append(f"  (el.{f} || '').trim()")
            elif f == "href":
                field_js_parts.append("  (el.href || el.getAttribute('href') || '')")
            elif f == "src":
                field_js_parts.append("  (el.src || el.getAttribute('src') || '')")
            elif f == "outerHTML":
                field_js_parts.append("  (el.outerHTML || '')")
            else:
                field_js_parts.append(
                    f"  (el.getAttribute('{f}') || el.{f} || '')")

        js = (
            "(() => {\n"
            f"  const els = document.querySelectorAll('{selector}');\n"
            "  const results = [];\n"
            f"  const limitNum = Math.min(els.length, {limit});\n"
            ...
            "    results.push(row);\n"
            "  }\n"
            f"  return {{ total: els.length, returned: limitNum, "
            f"results: results, fields: {_json.dumps(fields)} }};\n"
            "})()"
        )
        result = await self._ctrl.execute_js(js)
    ...
    async def extract_table(self, rows_selector: str,
        ...
        """从表格/列表结构结构化提取数据。"""
        self._step_count += 1
        cols_json = _json.dumps(columns, ensure_ascii=False)
        js = (
            "(() => {\n"
            f"  const rows = document.querySelectorAll('{rows_selector}');\n"
            f"  const cols = {cols_json};\n"
            "  const results = [];\n"
            f"  const limitNum = Math.min(rows.length, {limit});\n"
            "  for (let i = 0; i < limitNum; i++) {\n"
            "    const rowEl = rows[i];\n"
            "    const item = {};\n"
            "    for (const [name, sel] of Object.entries(cols)) {\n"
            "      const cell = rowEl.querySelector(sel);\n"
            "      item[name] = cell "
            "? (cell.innerText || cell.textContent || '').trim() : '';\n"
            "    }\n"
            "    results.push(item);\n"
            "  }\n"
            f"  return {{ total: rows.length, returned: limitNum, "
            f"results: results }};\n"
            "})()"
        )
        result = await self._ctrl.execute_js(js)
