...
class TestSessionCheck:
    def test_expired_cookie(self):
        from browser_agent.core.session import _is_expired
        past = time.time() - 3600
    ...
    def test_has_saved_cookies_no_file(self):
        from browser_agent.core.session import has_saved_cookies
        assert has_saved_cookies("nonexistent_test") is False

    async def test_load_storage_state_missing(self):
        from browser_agent.core.session import load_storage_state
