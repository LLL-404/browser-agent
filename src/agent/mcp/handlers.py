...
async def _save_state(agent, save_path: str) -> dict:
    ...
    try:
        ...
        return {"ok": True, "path": str(path), "cookies": len(state.get("cookies", [])), "size": path.stat().st_size}
    except Exception as e:
...
async def _load_state(agent, load_path: str) -> dict:
    ...
    try:
        ...
        return {"ok": True, "cookies_restored": len(data.get("cookies", [])), "origins_restored": len(data.get("origins", []))}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
async def _open(agent, ctrl, args):
    return await agent.open(headless=args.get("headless", False), url=args.get("url", "about:blank"))


async def _close(agent, ctrl, args):
    return await agent.close()

# ── Navigation ──


async def _navigate(agent, ctrl, args):
    return await agent.navigate(args.get("url", ""))


async def _navigate_back(agent, ctrl, args):
    ok = await ctrl.go_back()
    return {"ok": ok, "url": await ctrl.get_current_url() if ok else ""}


async def _resize(agent, ctrl, args):
...
async def _hover(agent, ctrl, args):
    sel = args.get("selector", "")
    ok = await ctrl.hover_selector(sel)
    return {"ok": ok, "selector": sel}


async def _select_option(agent, ctrl, args):
    sel = args.get("selector", "")
    val = args.get("value", "")
...
async def _fill_form(agent, ctrl, args):
...
async def _file_upload(agent, ctrl, args):
    sel = args.get("selector", "")
...
async def _press_key(agent, ctrl, args):
...
async def _snapshot(agent, ctrl, args):
...
async def _take_screenshot(agent, ctrl, args):
    return await agent.screenshot(full_page=args.get("full_page", False))


async def _text(agent, ctrl, args):
    return await agent.text()


async def _html(agent, ctrl, args):
    return await agent.html()


async def _url(agent, ctrl, args):
    return await agent.url()

# ── Scroll ──


async def _scroll(agent, ctrl, args):
...
async def _wait_navigation(agent, ctrl, args):
...
async def _wait_selector(agent, ctrl, args):
    sel = args.get("selector", "")
...
async def _handle_dialog(agent, ctrl, args):
...
async def _evaluate(agent, ctrl, args):
...
async def _cookie_list(agent, ctrl, args):
    cookies = await ctrl.save_cookies()
    return {"ok": True, "cookies": cookies, "count": len(cookies)}


async def _cookie_get(agent, ctrl, args):
    name_filter = args.get("name", "")
    cookies = await ctrl.save_cookies()
    found = [c for c in cookies if c.get("name") == name_filter]
    return {"ok": True, "name": name_filter, "cookie": found[0] if found else None, "found": len(found) > 0}


async def _cookie_set(agent, ctrl, args):
    page = ctrl._page
    if not page:
        return {"ok": False, "error": "browser not open"}
    cookie = {}
...
async def _storage_state(agent, ctrl, args):
    return await _save_state(agent, args.get("save_path", ""))


async def _storage_state_set(agent, ctrl, args):
    return await _load_state(agent, args.get("load_path", ""))


async def _localstorage_get(agent, ctrl, args):
    data = await ctrl.save_local_storage()
    return {"ok": True, "localStorage": data}


async def _localstorage_set(agent, ctrl, args):
    page = ctrl._page
    if not page:
        return {"ok": False, "error": "browser not open"}
    key = args.get("key", "")
...
async def _click_coordinate(agent, ctrl, args):
    page = ctrl._require_page()
...
async def _drag_coordinate(agent, ctrl, args):
    page = ctrl._require_page()
...
async def _hover_coordinate(agent, ctrl, args):
    page = ctrl._require_page()
    x, y = args.get("x", 0), args.get("y", 0)
    try:
...
async def _screenshot_save(agent, ctrl, args):
    page = ctrl._require_page()
    path = args.get("path", "")
...
async def _pdf_save(agent, ctrl, args):
    page = ctrl._require_page()
...
async def _devtools_dispatcher(agent, ctrl, args, tool_name: str):
    page = ctrl._require_page()
    try:
        ...
        if tool_name == "devtools_set_location":
            await cdp.send("Emulation.setGeolocationOverride", {
                "latitude": args.get("latitude", 0), "longitude": args.get("longitude", 0), "accuracy": 100,
            })
            return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}
