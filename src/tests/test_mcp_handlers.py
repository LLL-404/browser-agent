...
from agent.mcp.handlers import (
    TOOL_HANDLERS, TOOL_ALIASES,
    _open, _close, _navigate, _navigate_back,
    _click, _type, _hover, _select_option,
    _press_key, _snapshot, _take_screenshot,
    _text, _html, _url, _scroll,
    _wait_navigation, _wait_selector,
    _handle_dialog, _evaluate,
    _cookie_list, _cookie_get, _cookie_set,
    _storage_state, _storage_state_set,
    _localstorage_get, _localstorage_set,
    _console_messages, _network_requests,
    _file_upload, _fill_form,
    _resize,
    _click_coordinate, _drag_coordinate, _hover_coordinate, _screenshot_save,
    _pdf_save,
    _devtools_capture_profiling, _devtools_collect_garbage,
    _devtools_enable_device_emulation, _devtools_reset_device_emulation,
    _devtools_send_command, _devtools_set_location,
)
...
def mock_ctrl(mock_page):
    ...
    ctrl._require_page.return_value = mock_page
...
class TestRegistry:
    def test_all_handlers_registered(self):
        """All 42 handlers present in TOOL_HANDLERS."""
...
class TestInteraction:
    ...
    async def test_file_upload(self, mock_agent, mock_ctrl):
        r = await _file_upload(mock_agent, mock_ctrl, {"selector": "#file", "paths": ["/tmp/a.txt"]})
        assert r["ok"] is True
...
class TestVision:
    ...
    async def test_drag_coordinate(self, mock_agent, mock_ctrl, mock_page):
        r = await _drag_coordinate(mock_agent, mock_ctrl, {"start_x": 0, "start_y": 0, "end_x": 100, "end_y": 100})
        assert r["ok"] is True
